...
class StormsInfo:
    ...
    def GetStormLabels(self, basinLabel, year):
        '''This function is going away, may already be deprecated.'''
        with open(r".\hurrevac_settings.json") as f:
            hurrevacSettings = json.load(f)
        StormBasins = hurrevacSettings['BasinsDictionary']
        ...
        exerciseStorms.sort()
        simulatedStorms.sort()
        if hurrevacSettings['ShowStormTypes']['Active'] == 1:
            for i in activeStorms:
                stormNameIdStatusList.append(i)
        if hurrevacSettings['ShowStormTypes']['Historical'] == 1:
            for i in historicStorms:
                stormNameIdStatusList.append(i)
        if hurrevacSettings['ShowStormTypes']['Exercise'] == 1:
            for i in exerciseStorms:
                stormNameIdStatusList.append(i)
        if hurrevacSettings['ShowStormTypes']['Simulated'] == 1:
            for i in simulatedStorms:
                stormNameIdStatusList.append(i)
