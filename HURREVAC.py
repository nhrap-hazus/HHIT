...
class Hurrevac:
    ...
    def getStormList(self):
        stormsDict = []
        years = getYears(jsonData)
        for year in years:
            for storm in jsonData[str(year)]:
                stormYear = int(year)
        ...
        self.StormList = dfStorms.sort_values(by=['stormYear'])
    ...
    def exportToJSON(self, outputPath):
        df = self.stormData
        #this isn't exporting good data yet...
